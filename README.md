...
- **智能填表与语音**：排盘表单支持说/打一句话（"我是92年农历八月初八晚上十点一刻在上海出生的女生"），豆包结构化抽取自动填表并提示缺失字段；排盘表单与随盘追问均支持语音输入（浏览器 Web Speech API，Edge/Safari/Chrome，不支持的浏览器自动隐藏麦克风按钮）。
- **对话记忆**：追问按命盘维度持久化，上下文携带命盘数据 + 最新报告 + 最近 20 条历史消息。
